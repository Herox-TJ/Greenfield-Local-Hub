"""Run once to populate the database with the sample data below.
   Usage: python seed.py
   Credentials:
     Admin:    admin@greenfield.com  / admin123
     Customer: customer@greenfield.com / customer123
     Producer: josh@greenvalley.com   / producer123
"""

from app import app, db
from model import User, Producer, Product, CartItem, Order, OrderItem
...
def seed():
    with app.app_context():
        ...
        customer = User(first_name='TJ', last_name='Swami',
                        ...
                        security_question='What was the name of your first pet?',
                        security_answer='buddy', loyalty_points=0)
        ...
        prod_user = User(first_name='Josh', last_name='Green',
                         ...
                         security_question='What was the name of your first pet?',
                         security_answer='max')
        prod_user.set_password('producer123')

        db.session.add_all([admin, customer, prod_user])
        db.session.flush()
        ...
        products = [
            Product(producer_id=gvf.id, name='Organic Carrots',
                    slug='organic-carrots',
                    description='Sweet, crunchy carrots grown in rich organic soil.',
                    price=2.50, unit='1kg', stock=50, category='Vegetables',
                    image=PI2 + imgs[0]),
            Product(producer_id=gvf.id, name='Fresh Spinach',
                    slug='fresh-spinach',
                    description='Tender baby spinach leaves, perfect for salads.',
                    price=3.00, unit='250g', stock=30, category='Vegetables',
                    image=PI2 + imgs[1]),
            Product(producer_id=gvf.id, name='Heritage Tomatoes',
                    slug='heritage-tomatoes',
                    description='Mixed variety heritage tomatoes with rich flavour.',
                    price=4.50, unit='500g', stock=25, category='Vegetables',
                    image=PI2 + imgs[2]),
            Product(producer_id=gvf.id, name='Organic Potatoes',
                    slug='organic-potatoes',
                    description='Versatile potatoes ideal for roasting or mashing.',
                    price=2.00, unit='2kg', stock=100, category='Vegetables',
                    image=PI2 + imgs[3]),
            Product(producer_id=so.id, name='Fresh Apples',
                    slug='fresh-apples',
                    description='Crisp, juicy apples picked at peak ripeness.',
                    price=3.50, unit='1kg', stock=80, category='Fruits',
                    image=PI2 + imgs[4]),
            Product(producer_id=so.id, name='Ripe Pears',
                    slug='ripe-pears',
                    description='Soft, sweet pears great for eating or cooking.',
                    price=4.00, unit='1kg', stock=60, category='Fruits',
                    image=PI2 + imgs[5]),
            Product(producer_id=so.id, name='Fresh Strawberries',
                    slug='fresh-strawberries',
                    description='Sweet, juicy strawberries, freshly picked.',
                    price=5.00, unit='400g', stock=40, category='Fruits',
                    image=PI2 + imgs[6]),
            Product(producer_id=mdc.id, name='Whole Milk',
                    slug='whole-milk',
                    description='Fresh whole milk from our grass-fed herd.',
                    price=1.80, unit='2L', stock=50, category='Dairy',
                    image=PI2 + imgs[7]),
            Product(producer_id=mdc.id, name='Farmhouse Cheddar',
                    slug='farmhouse-cheddar',
                    description='Mature cheddar cheese aged for rich flavour.',
                    price=8.50, unit='400g', stock=30, category='Dairy',
                    image=PI2 + imgs[8]),
            Product(producer_id=mdc.id, name='Fresh Butter',
                    slug='fresh-butter',
                    description='Churned butter made fresh from our dairy herd.',
                    price=3.20, unit='250g', stock=45, category='Dairy',
                    image=PI2 + imgs[9]),
            Product(producer_id=hb.id, name='Sourdough Loaf',
                    slug='sourdough-loaf',
                    description='Traditional sourdough with a chewy crust.',
                    price=4.00, unit='800g', stock=20, category='Bakery',
                    image=PI2 + imgs[10]),
            Product(producer_id=hb.id, name='Wholemeal Bread',
                    slug='wholemeal-bread',
                    description='Nutritious wholemeal bread with hearty texture.',
                    price=3.50, unit='750g', stock=25, category='Bakery',
                    image=PI2 + imgs[11]),
            Product(producer_id=rp.id, name='Grass-Fed Beef Mince',
                    slug='grass-fed-beef-mince',
                    description='Lean grass-fed beef mince from free-range cattle.',
                    price=8.50, unit='500g', stock=35, category='Meat',
                    image=PI2 + imgs[12]),
            Product(producer_id=rp.id, name='Lamb Chops',
                    slug='lamb-chops',
                    description='Tender lamb chops from pasture-raised animals.',
                    price=12.00, unit='500g', stock=20, category='Meat',
                    image=PI2 + imgs[13]),
        ]
        ...
        print('  customer@greenfield.com / customer123')
        print('  josh@greenvalley.com   / producer123')
